...
contains
  ...
  subroutine read_HYCOM_SST(filename,gr,data)
    use mod_measurement
    use mod_grid
    use m_spherdist
    use netcdf
    ...
    if (ex) then
       ...
       call nfw_inq_varid(filename, ncid,'thetao' ,vsst_ID)
#if defined (CCI_SST)
       ...
       do i=1,gr%nx
          do j=1,gr%ny
            ...
            !
            !print *, 'i:', i, ', j:', j, 'lat(j):', lat(j), ',sst(i,j):', sst(i,j)                 
            if (lat(j) > -30 .and.&
  ...
  end subroutine read_HYCOM_SST
end module m_read_HYCOM_SST
